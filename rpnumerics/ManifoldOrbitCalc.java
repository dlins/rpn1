...
public class ManifoldOrbitCalc implements RpCalculation {
    ...
    public PhasePoint orbitRefPoint() { return firstPoint_; }
    ...
    public RpSolution calc() throws RpException {
        ...
        if (correctInput) {
            if ((stationaryPoint_.getIntegrationFlag() != RpSolution.CONVERGENCE_IN_STATIONARY_POINT_COMPUTATION) ||
                (stationaryPoint_.getDimP() == 0) || (stationaryPoint_.getDimN() == 0)) correctInput = false;
        }
        //check first point
        if (correctInput) {
            int kk;
        ...
        if (!correctInput) { throw new RpException("wrong input"); } // incorrect input
        ...
        HessianMatrix D2X = RPNUMERICS.flow().fluxDeriv2(point0);
        for (int v = 0; v < m; v++)
            for (int w = 0; w < m; w++)
                for (int u = 0; u < m; u++) {
	...
	try{
		...
		RealVector[] coords = solution.getCoordinates();
    ...
    protected RealMatrix2 sylvester(int n1, int n2, RealMatrix2 A, RealMatrix2 B, RealMatrix2 C) { // was changed 12.11.2003
        ...
        UA.convert(n1, n1, schurVecArrayA);
        UB.convert(n2, n2, schurVecArrayB);
