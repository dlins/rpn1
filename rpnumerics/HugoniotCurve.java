...
public class HugoniotCurve extends RPnCurve implements RpSolution {
    ...
    public HugoniotCurve(PhasePoint xZero, WaveState[] states) {
        super(coordsArrayFromRealSegments(HugoniotCurve.
                                          hugoniotSegsFromWaveState(xZero,
                states)), new ViewingAttr(Color.red));
        //        segments_ = new ArrayList();
        xZero_ = new PhasePoint(xZero);
        hugoniotSegments_ = HugoniotCurve.hugoniotSegsFromWaveState(xZero, states);

        //        int inputSize = states.length;
        //        for (int i = 0; i < inputSize - 1; i++) {
        //            // type is set...
        //            HugoniotPoint v1 = new HugoniotPoint(xZero_, states[i].finalState().getCoords(), states[i].speed());
        //            HugoniotPoint v2 = new HugoniotPoint(xZero_, states[i + 1].finalState().getCoords(), states[i + 1].speed());
        //            if ((v1.type().equals(v2.type())))
        //                segments_.add(new HugoniotSegment(v1, v1.sigma(), v2, v2.sigma(), v1.type()));
        //            else {
        //                List partCurve = interpolate(v1, v2);
        //                segments_.addAll(partCurve);
        //            }
        //        }
    }
...
//              HugoniotSegment segment = (HugoniotSegment)segments_.get(i);
    ...
    private static List hugoniotSegsFromWaveState(PhasePoint
            xZero, WaveState[] wStates) {
        ...
        for (int i = 0; i < inputSize - 1; i++) {
            ...
            HugoniotPoint v1 = new HugoniotPoint(xZero,
                                                 wStates[i].finalState().getCoords(),
                                                 wStates[i].speed());
    ...
    private static List hugoniotSegsFromRealSegs(PhasePoint xZero_, List realSegs) {
    ...
    // Accessors/Mutators
    //
    public List segments() { return hugoniotSegments_; }
    ...
    public double findSigma(PhasePoint targetPoint) {
        ...
        //     HugoniotSegment segment = (HugoniotSegment)segments_.get(hugoniotSegmentIndx);
