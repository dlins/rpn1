...
(JNIEnv * env, jobject obj, jint waveCurveID) {
    ...
    HugoniotContinuation * hug = RpNumerics::physicsVector_->at(0)->Hugoniot_continuation();
    
    
    
    WaveCurveFactory * wavecurvefactory = RpNumerics::physicsVector_->at(0)->wavecurvefactory();
    ...
    for (int n = 0; n < waveCurveVector.size(); n++) {
        ...
        RpNumerics::addWaveCurve(CURVEID,new WaveCurve(waveCurveVector[n]));
