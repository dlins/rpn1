...
(JNIEnv * env, jobject obj, jobject uMinus, jintArray resolution) {
    ...
    cout <<"Valor de Uref na chamada antes de preprocess: "<<Uref<<endl;
...
//    cout << "Teste" << testeBoundary->minimums() << " " << testeBoundary->maximums() << endl;
    ...
    cout <<"Ultimo valor do grid: "<<gv->grid(gv->grid.rows()-1,gv->grid.cols()-1)<<endl;
    ...
    hugoniotCurve->classified_curve(&RpNumerics::getPhysics().fluxFunction(), &RpNumerics::getPhysics().accumulation(), *gv, Uref, hugoniotPolyLine);
    ...
    hugoniotCurve.classified_curve(&RpNumerics::getPhysics().fluxFunction(), &RpNumerics::getPhysics().accumulation(),
            *gv, Uref, hugoniotPolyLineVector);

  

    for (int i = 0; i < hugoniotPolyLineVector.size(); i++) {

        cout << "Tamanho do ramo : " << i << " " << hugoniotPolyLineVector[i].point.size() << endl;

    }


        RpNumerics::getPhysics().getSubPhysics(0).postProcess(hugoniotPolyLine[i].point[0]);
        RpNumerics::getPhysics().getSubPhysics(0).postProcess(hugoniotPolyLine[i].point[1]);


        jdoubleArray eigenValRLeft = env->NewDoubleArray(dimension);
        jdoubleArray eigenValRRight = env->NewDoubleArray(dimension);

        double * leftCoords = (double *) hugoniotPolyLine[i].point[0];


        double * rightCoords = (double *) hugoniotPolyLine[i].point[1];
...
=======
//        cout << "segmento: " << hugoniotPolyLine[i].point[0] << " " << hugoniotPolyLine[i].point[1] << endl;
>>>>>>> tpcwMergeMaster5


        double leftSigma = hugoniotPolyLine[i].speed[0];
        double rightSigma = hugoniotPolyLine[i].speed[1];

        double leftLambda1 = hugoniotPolyLine[i].eigenvalue[0].component(0);
        double leftLambda2 = hugoniotPolyLine[i].eigenvalue[0].component(1);

        double rightLambda1 = hugoniotPolyLine[i].eigenvalue[1].component(0);
        double rightLambda2 = hugoniotPolyLine[i].eigenvalue[1].component(1);
