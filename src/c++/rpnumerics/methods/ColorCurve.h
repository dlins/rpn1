...
// TODO: Inserir a matriz de viscosidade no calculo das transicoes de cores. A classificacao de Lax, que depende da matriz de viscosidade, muda.
...
class ColorCurve {
    ...
    void classify_segmented_curve(std::vector<RealVector>  &original, /*const RealVector &ref*/ const ReferencePoint &ref,
                                  std::vector<HugoniotPolyLine> &classified_curve,
                                  std::vector<RealVector> &transition_list);

    void classify_continuous_curve(std::deque<RealVector>  &original, /*const RealVector &ref*/ const ReferencePoint &ref,
