...
Extension_Curve::Extension_Curve(
        ...
        const FluxFunction *dff, const AccumulationFunction *daa, const Boundary *dbb) {
    ...
    domain_is_inside.resize(domain_rows, domain_cols);
...
void Extension_Curve::curve2p5(int characteristic_where,
    ...
    for (int i = 0; i < nur; i++) {
        ...
        for (int j = 0; j < nvr; j++) {
            ...
            // Skip the whole square if one point is outside.
            if (!domain_is_inside(i + 1, j + 1)) continue;
...
void Extension_Curve::set_domain_family(int family) {
    ...
    validate_cells(domain_family, domain_cell_type, domain_eig_is_real, domain_is_complex,domain_is_inside);
