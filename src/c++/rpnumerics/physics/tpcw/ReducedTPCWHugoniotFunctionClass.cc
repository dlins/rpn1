...
//HugoniotFunctionClass * ReducedTPCWHugoniotFunctionClass::clone() {
...
//
//    // Create the auxiliary objects
//    ReducedFlux2Comp2PhasesAdimensionalized_Params ReducedTPCWFluxAdimensionalized_Params(copy.abs_copy.perm, td, fh);
//    ReducedTPCWFluxAdimensionalized = new ReducedFlux2Comp2PhasesAdimensionalized(ReducedTPCWFluxAdimensionalized_Params);
//
//    ReducedAccum2Comp2PhasesAdimensionalized_Params ReducedTPCWAccumAdimensionalized_Params(td, &phi);
//    ReducedTPCWAccumAdimensionalized = new ReducedAccum2Comp2PhasesAdimensionalized(ReducedTPCWAccumAdimensionalized_Params);
...
void ReducedTPCWHugoniotFunctionClass::CompleteHugoniot(double &shock_speed, double &vel_plus, const RealVector &uplus) {
    ...
    ReducedTPCWFluxAdimensionalized->jet(uplus, aJetMatrix, 0);
    ReducedTPCWAccumAdimensionalized->jet(uplus, bJetMatrix, 0);
    ...
    double X13minus = F1minus * G3quad - F3minus*G1quad;
    double X23minus = F2minus * G3quad - F3minus*G2quad;

    double X12plus = F1plus * G2quad - F2plus*G1quad;
    double X13plus = F1plus * G3quad - F3plus*G1quad;
    double X23plus = F2plus * G3quad - F3plus*G2quad;

    double Y12 = F1plus * F2minus - F1minus*F2plus;
    double Y13 = F1plus * F3minus - F1minus*F3plus;
    double Y23 = F2plus * F3minus - F2minus*F3plus;


    double den = X12plus * X12plus + X13plus * X13plus + X23plus*X23plus;

    shock_speed = (Y12 * X12plus + Y13 * X13plus + Y23 * X23plus) / den;

    vel_plus = X12minus * X12plus + X13minus * X13plus + X23minus*X23plus;
