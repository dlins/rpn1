...
void SubinflectionTP::subinflection_function(double & reduced_lambdae, double & numeratorchiu,
    ...
    JetMatrix rhosigmacj(1);
    td->Rhosic_jet(Theta, 2, rhosigmacj);
    double rhosigmac       = rhosigmacj.get(0);
    double drhosigmac_dT   = rhosigmacj.get(0, 0);
    double d2rhosigmac_dT2 = rhosigmacj.get(0, 0, 0);

    JetMatrix rhosigmawj(1);
    td->Rhosiw_jet(Theta, 2, rhosigmawj);
    double rhosigmaw       = rhosigmawj.get(0);
    double drhosigmaw_dT   = rhosigmawj.get(0, 0);
    double d2rhosigmaw_dT2 = rhosigmawj.get(0, 0, 0);

    JetMatrix rhoacj(1);
    td->Rhoac_jet(Theta, 2, rhoacj);
    double rhoac       = rhoacj.get(0);
    double drhoac_dT   = rhoacj.get(0, 0);
    double d2rhoac_dT2 = rhoacj.get(0, 0, 0);

    JetMatrix rhoawj(1);
    td->Rhoaw_jet(Theta, 2, rhoawj);
    double rhoaw       = rhoawj.get(0);
    double drhoaw_dT   = rhoawj.get(0, 0);
    double d2rhoaw_dT2 = rhoawj.get(0, 0, 0);

    JetMatrix Haj(1);
    td->AqueousEnthalpyVol_jet(Theta, 2, Haj);
    double Ha       = Haj.get(0);
    double dHa_dT   = Haj.get(0, 0);
    double d2Ha_dT2 = Haj.get(0, 0, 0);

    JetMatrix Hsij(1);
    td->SuperCriticEnthalpyVol_jet(Theta, 2, Hsij);
    double Hsi       = Hsij.get(0);
    double dHsi_dT   = Hsij.get(0, 0);
    double d2Hsi_dT2 = Hsij.get(0, 0, 0);

    JetMatrix Hrj(1);
    td->RockEnthalpyVol_jet(Theta, 2, Hrj);
    double Hr       = Hrj.get(0);
    double dHr_dT   = Hrj.get(0, 0);
    double d2Hr_dT2 = Hrj.get(0, 0, 0);
