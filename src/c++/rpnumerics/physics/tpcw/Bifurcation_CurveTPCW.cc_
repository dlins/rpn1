...
void Bifurcation_CurveTPCW::fill_values_on_grid(const RealVector &pmin, const RealVector &pmax, 
				            ...
				            //  TODO: Trazer as classes acima (de fora pelo construtor)
    ...
    for (int i = 0; i < n; i++){
        ...
        Eigen::eig(dimP, &JF[0][0], &JG[0][0], etemp); // TODO: NAO PRECISA DE AUTOVETOR!

        e(i).clear();
        e(i).resize(etemp.size());
        for (int j = 0; j < etemp.size(); j++) e(i)[j] = etemp[j]; // TODO: ESCLARESCER ESTA LINHA
...
void Bifurcation_CurveTPCW::fill_values_on_curve(const FluxFunction *ff, const AccumulationFunction *aa, const std::vector<RealVector> &input, 
