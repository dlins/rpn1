...
int Stone_Explicit_Bifurcation_Curves::cross_sec_bif(const RealVector &previous_point, const RealVector &point, RealVector &crossing_point, int &region){
    ...
    double eq_SW = so*mug - (1.0 - sw - so)*muo;
    double eq_SO = sw*mug - (1.0 - sw - so)*muw;
    double eq_SG = so*muw - sw*muo;
...
void Stone_Explicit_Bifurcation_Curves::sec_bif_correspondence(int side_opposite_vertex, int nos, 
    ...
    if (expw  == 2.0 && expg  == 2.0 && expo == 2.0 &&
        ...
        points_to_segments(point);
        points_to_segments(correspondent_point);
...
void Stone_Explicit_Bifurcation_Curves::points_to_segments(std::vector<RealVector> &v){
