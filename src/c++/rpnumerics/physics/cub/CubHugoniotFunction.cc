...
CubHugoniotFunction::CubHugoniotFunction(const RealVector &U, const Cub2FluxFunction & fluxFunction) : HugoniotFunctionClass(fluxFunction) {
...
CubHugoniotFunction::~CubHugoniotFunction() {
    //    delete stone;
}

void CubHugoniotFunction::setReferenceVector(const RealVector & refVec) {
    Cub2FluxFunction & fluxFunction = (Cub2FluxFunction &) getFluxFunction();
    Uref = refVec;
    // TODO: The flux object must be initialized somehow (be it created here or outside, etc.)
...
double CubHugoniotFunction::HugoniotFunction(const RealVector & U) {
