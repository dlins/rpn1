...
//    // This was a test that misfired. Splines cannot be replaced by a fitting made
//    // up by a sum of sines without sacrificing speed.

//    // The data corresponding to hsigmaC is fitted using the function
//    //
//    //     h = A(0)*sin(B(0)*T + C(0)) + ... + A(7)*sin(B(7)*T + C(7)).
//    //
//    // Thus, splines are no longer necessary.
//    //
//    double A_data[8] = { 0.629322462003440,  0.153151918580917,  0.062846217409066,
//                         1.790942413366250,  0.030314673629487,  0.017789951511209,
//                         0.035865232407301,  0.029673333036949};

//    double B_data[8] = { 0.000000416290019,  0.000000840380178,  0.000001283458418,
//                         0.000000180094400,  0.000001736477024,  0.000002235648644,
//                         0.000002748627385,  0.000002835331653};

//    double C_data[8] = {-0.000005998986346, -0.000014935753596,  0.000032711230000,
//                        -0.000006354716100,  0.000014107037042, -0.000022200704479,
//                        -0.000064136464048,  0.000059147771265};

//    int n = 8;

//    A = 1.0e+5*RealVector(n, A_data);
//    B = 1.0e+5*RealVector(n, B_data);
//    C = 1.0e+5*RealVector(n, C_data);

//    A_times_B.resize(n);
//    minus_A_times_B_times_B.resize(n);

//    for (int i = 0; i < n; i++){
//        A_times_B(i) = A(i)*B(i);
//        minus_A_times_B_times_B(i) = -A_times_B(i)*B(i);
//    }
}
...
int Thermodynamics::hsigmaC_jet(const double Theta, int degree, JetMatrix &hsigmaCj){
    ...
    double b = -8.75e5; // TEST ONLY!!!
    double m = 2274.54; // TEST ONLY!!!

    if (degree >= 0){
        hsigmaCj.set(0, h/h_typical_);
        hsigmaCj.set(0, (b + m*T)/h_typical_); // TEST ONLY!!!


        if (degree >= 1){
            hsigmaCj.set(0, 0, d_h*T_typical_/h_typical_);
            hsigmaCj.set(0, 0, m*T_typical_/h_typical_); // TEST ONLY!!!

            if (degree == 2){
                hsigmaCj.set(0, 0, 0, d2_h*T_typical_*T_typical_/h_typical_);
                hsigmaCj.set(0, 0, 0, 0.0); // TEST ONLY!!!
            }
        }
    }
...
// Sine fitting: not working as intended.
//
//
...
//        RealVector sin_evald(n);

//        for (int i = 0; i < n; i++){
//            sin_evald(i) = sin(B(i)*T + C(i));
...
//                for (int i = 0; i < n; i++){
//                    d2_h += minus_A_times_B_times_B(i)*sin_evald(i);
//                }
