...
StoneNegativePermeability::StoneNegativePermeability(const StoneNegativePermParams & params) : params_(new StoneNegativePermParams(params)) {
...
StoneNegativePermeability::StoneNegativePermeability(const StoneNegativePermeability & copy) : params_(new StoneNegativePermParams(copy.params())) {
...
StoneNegativePermeability::~StoneNegativePermeability() {
    delete params_;
}

void StoneNegativePermeability::Diff_PermabilityWater(double sw, double so, double sg,
 ...
 *  mixture of models is possible: epsl = 0 means Corey, epsl = 1, StoneNegative.
 *     lo, low, lw, log, etcetera represent linear against the mixtured Corey-StoneNegative model.
 *  In the same spirit of ideas for saturations below the critical ones, we expand the
 *  StoneNegative model with the value for that saturation as the critical. This means, if water
 ...
 *          /  \/\
 *         /   /\ \
 *        /   /  \ \       <-- Region 2 has the StoneNegative with general powers.
...
void StoneNegativePermeability::Diff_PermabilityOil(double sw, double so, double sg,
    ...
    if (!NegativeOilSaturation && (socno <= 0.)) {
        ...
        double powso1 = socno*powso2;
        double StoneNegativeC = epsl_ * kro_p_ * (1. - CN);

        ko         = StoneNegativeC * socno * ko1den * ko2den
                   + (1. - epsl_)*(lo_ + (1. - lo_) * powso1) * socno * denko_;
        dko_dsw    = StoneNegativeC * socno * dko_Aux;
                   // Plus zero Corey contribution.
        dko_dso    = StoneNegativeC * ko1den * (ko2den + socno * dko2den_ds)
                   + (1. - epsl_)*(lo_ + (1. - lo_) * expo_ * powso1) * denko_;
        d2ko_dsw2  = StoneNegativeC * socno * d2ko_Aux;
                   // Plus zero Corey contribution.
        d2ko_dswso = StoneNegativeC * (dko_Aux + socno * dko_Aux2);
                   // Plus zero Corey contribution.
        d2ko_dso2  = StoneNegativeC * ko1den * (2. * dko2den_ds + socno * d2ko2den_ds2)
...
void StoneNegativePermeability::Diff_PermabilityGas(double sw, double so, double sg, 
