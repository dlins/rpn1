...
//        double XTranslateFactor = -(viewPlane().getWindow().getOriginPosition().x + viewPlane().getWindow().getOriginPosition().x*.5) * XScaleFactor;
        double XTranslateTerm = -(viewPlane().getWindow().getOriginPosition().x) * XScaleFactor;
        // we are working with RASTER
        double YTranslateTerm = -viewPlane().getWindow().getOriginPosition().y * YScaleFactor
	...
	double YShearFactor = 0.*YScaleFactor;
        ...
        XTranslateTerm += viewPlane().getViewport().getOriginPosition().x;
        YTranslateTerm += viewPlane().getViewport().getOriginPosition().y;
        ...
        coordSysTransform().getTransfMatrix().setElement(2, 0, XTranslateTerm);
        coordSysTransform().getTransfMatrix().setElement(2, 1, YTranslateTerm);
