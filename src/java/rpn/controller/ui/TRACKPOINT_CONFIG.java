...
public class TRACKPOINT_CONFIG extends UI_ACTION_SELECTED {
    ...
    public void trackPoint(RPnPhaseSpacePanel panel, Coords2D dcCoords) {
        ...
        CoordsArray worldCoords = new CoordsArray(new Space("", rpnumerics.RPNUMERICS.domainDim() * 2));

        panel.scene().getViewingTransform().dcInverseTransform(dcCoords, worldCoords);
        int[] indices = panel.scene().getViewingTransform().projectionMap().getCompIndexes();
    ...
    private void trackedPoint(RealVector nDPoint) {
        ...
        for (int i = 0; i < RPnUIFrame.getAuxFrames().length; i++) {
            ...
            RPnPhaseSpacePanel panel = RPnUIFrame.getAuxFrames()[i].phaseSpacePanel();
            Coords2D coords2D = new Coords2D();
            CoordsArray worldCoords = new CoordsArray(nDPoint);
