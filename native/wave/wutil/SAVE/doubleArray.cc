...
#endif
// CHANGE:
// OLD: #include <stream.h>
// OLD: #include <builtin.h>
#include <stdlib.h>
#include <iostream.h>
// END CHANGE
...
one_arg_error_handler_t doubleArray_error_handler = default_doubleArray_error_handler;
...
void doubleArray::error(const char* msg)
{
  (*doubleArray_error_handler)(msg);
}

// CHANGE:
// OLD: void doubleArray::range_error()
void doubleArray::range_error() const
// END CHANGE
{
  (*doubleArray_error_handler)("Index out of range.");
}

// CHANGE:
// OLD: doubleArray::doubleArray(doubleArray& v)
doubleArray::doubleArray(const doubleArray& v)
// END CHANGE
{
...
doubleArray::doubleArray(int l, double  fill_value)
{
  s = new double [len = l];
  double* top = &(s[len]);
  double* t = s;
...
// CHANGE:
// OLD: doubleArray& doubleArray::operator = (doubleArray& v)
doubleArray& doubleArray::operator = (const doubleArray& v)
// END CHANGE
{
  if (this != &v)
  {
...
void doubleArray::apply(doubleProcedure f)
{
  double* top = &(s[len]);
  double* t = s;
  while (t < top) (*f)(*t++);
...
void doubleArray::resize(int newl)
{
...
doubleArray concat(doubleArray & a, doubleArray & b)
{
  ...
  t = b.s;
  while (t < top) *p++ = *t++;
  return doubleArray(newl, news);
}


doubleArray combine(doubleCombiner f, doubleArray& a, doubleArray& b)
{
...
double doubleArray::reduce(doubleCombiner f, double  base)
{
  double r = base;
  double* top = &(s[len]);
  double* t = s;
...
doubleArray reverse(doubleArray& a)
{
  ...
  return doubleArray(a.len, news);
}

void doubleArray::reverse()
{
  if (len != 0)
  {
    double* lo = s;
...
int doubleArray::index(double  targ)
{
// CHANGE:
// OLD: for (int i = 0; i < len; ++i) if (doubleEQ(targ, s[i])) return i;
  for (int i = 0; i < len; ++i) if (targ == s[i]) return i;
// END CHANGE
  return -1;
}

doubleArray map(doubleMapper f, doubleArray& a)
{
  double* news = new double [a.len];
  double* p = news;
  double* top = &(a.s[a.len]);
...
int operator == (doubleArray& a, doubleArray& b)
{
  if (a.len != b.len)
    return 0;
  double* top = &(a.s[a.len]);
  ...
  double* u = b.s;
// CHANGE:
// OLD: while (t < top) if (!(doubleEQ(*t++, *u++))) return 0;
  while (t < top) if (!(*t++ == *u++)) return 0;
// END CHANGE
  return 1;
}

void doubleArray::fill(double  val, int from, int n)
{
  int to;
  if (n < 0)
    to = len - 1;
...
doubleArray doubleArray::at(int from, int n)
{
  int to;
  if (n < 0)
  {
  ...
  double* top = &(s[to]);
  while (t <= top) *p++ = *t++;
  return doubleArray(n, news);
}

doubleArray merge(doubleArray & a, doubleArray & b, doubleComparator f)
{
  int newl = a.len + b.len;
  double* news = new double [newl];
  double* p = news;
...
void doubleArray::sort (doubleComparator compar)
{
  gsort(s, len, compar);
}
