...
public class Rk4BPMethod implements ODESolver {
    ...
    public ODESolution solve(RealVector firstPoint, int timeDirection) {
        ...
        times[0] = 0;
        int stepN = 1;
        while ((stepN < opt_.getMaximumStepNumber()) && (resultFlag == MADE_MAXIMUM_STEP_NUMBER)) {
            hNew = rkqc(y, h, yNew);
            // boundary
        ...
        RealVector[] resultCoords = new RealVector[stepN];
        double[] resultTimes = new double[stepN];
        for (int i = 0; i < stepN; i++) {
            resultCoords[i] = new RealVector(coords[i]);
    ...
    // Accessors
    //
    public ODESolverProfile getProfile() { return opt_; }
    ...
    protected double norm(RealVector y) {
        ...
        for (int i = 0; i < y.getSize(); i++)
            normValue += y.getElement(i) * y.getElement(i) /
                (opt_.getYScales().getElement(i) * opt_.getYScales().getElement(i));
