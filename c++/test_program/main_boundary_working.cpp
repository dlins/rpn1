...
//    yref->value(yinput->value());
    ...
    for (int i = 0; i < rarcurve.size(); i++) p.push_back(Point2D(rarcurve[i].component(0), rarcurve[i].component(1)));

    Curve2D *curve2d = new Curve2D(p, 1, 0, 0);
    canvas->add(curve2d);

    char char_info[100];
    sprintf(char_info, "Init = (%f, %f). %s. Family = %d. Total number of points: %d", x, y, (increase->value()) ? "Forward" : "Backward", (int)curve_family->value(), rarcurve.size());
...
//                domain_family->hide();
    ...
    canvaswin = new Fl_Double_Window(max_canvas_size, max_canvas_size, "Rarefaction (SinglePhase)");
    {
