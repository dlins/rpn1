...
// TODO: All these members should pass as parameters, that the user may change them.
//
MolarDensity::MolarDensity(int t, Parameter *Pressure): type(t), P_parameter(Pressure){
    init();
}
...
int  MolarDensity::epsilon_c_jet(const double T, int degree, JetMatrix &epsilon_cj){
	...
	double kappa_c	      = ( kappa_0c + kappa_1c * (1.0 + sqrt_Tr) * (0.7 - T_red) );
         
        //double alpha_c = 1.0 + ( kappa_0c + kappa_1c * (1.0 + sqrt_Tr) * (0.7 - T_red) ) * (1.0 - sqrt_Tr);        
	double alpha_c = 1.0 + kappa_c * (1.0 - sqrt_Tr);
        double epsilon_c = epsilon_factor * alpha_c * alpha_c * inv_T;
	...
	// Jet de T_red; 
        // double dT_red_dT      = 1/Tcc;
	// double d2T_red_dT2    = 0;   

	//Derivatives of sqrt_Tr   
        double dsqrt_Tr_dT    =   0.5*(1.0/sqrt_Tr)*(1.0/Tcc) ;
        double d2sqrt_Tr_dT2  =   -0.25*( 1.0/(sqrt_Tr*T_red) )*(1.0/Tcc)*(1.0/Tcc);

        //Derivatives of kappa_c 
	//double kappa_c      =   kappa_0c + kappa_1c * (1.0 + sqrt_Tr) * (0.7 - T_red)   
	double dkappa_c_dT    =   kappa_1c * (dsqrt_Tr_dT) * (0.7 - T_red) + kappa_1c * (1.0 + sqrt_Tr) * (-1.0/Tcc) ;
	double d2kappa_c_dT2  =   kappa_1c * (d2sqrt_Tr_dT2) * (0.7 - T_red) + 2.0*kappa_1c * (dsqrt_Tr_dT) * (-1.0/Tcc) ;  			   	
        
	//Derivatives of alpha_c
	//double alpha_c      =   1.0 + kappa_c * (1.0 - sqrt_Tr);
	double dalpha_c_dT    =   dkappa_c_dT * (1.0 - sqrt_Tr) + kappa_c * (-dsqrt_Tr_dT);
	double d2alpha_c_dT2  =   d2kappa_c_dT2 * (1.0 - sqrt_Tr) + 2.0*dkappa_c_dT * (-dsqrt_Tr_dT) + kappa_c * (-d2sqrt_Tr_dT2);
			    

        if (degree >= 1){

            double depsilon_c_dT = epsilon_factor*( 2.0*alpha_c*dalpha_c_dT*inv_T - alpha_c*alpha_c*inv_T2 );
            ...
            if (degree == 2){
                ...
                double d2epsilon_c_dT2 = epsilon_factor*(   2.0*(dalpha_c_dT*dalpha_c_dT + alpha_c*d2alpha_c_dT2)*inv_T - 2.0*alpha_c*dalpha_c_dT*inv_T2 - ( 2.0*alpha_c*dalpha_c_dT*inv_T2 - 2.0*alpha_c*alpha_c*inv_T3 )  );
...
int  MolarDensity::epsilon_w_jet(const double T, int degree, JetMatrix &epsilon_wj){
    ...
    if (degree >= 0){
        ...
        double T_red          = T/Tcw;
        double sqrt_Tr        = sqrt(T_red);
        double epsilon_factor = 0.457235 * Tcw / 0.077796; // It represents aw/(bw*R)
        double kappa_0w       = 0.378893 + 1.4897153*omega_w - 0.17131848*omega_w*omega_w + 0.0196554*omega_w*omega_w*omega_w;
        double inv_T          = 1.0/T;
	double inv_T2 	      = inv_T*inv_T ;
	double inv_T3         = inv_T2*inv_T ; 
	double kappa_w	      = ( kappa_0w + kappa_1w * (1.0 + sqrt_Tr) * (0.7 - T_red) );	
         
//        double alpha_w = 1.0 + ( kappa_0w + kappa_1w * (1.0 + sqrt_Tr) * (0.7 - T_red) ) * (1.0 - sqrt_Tr);
	double alpha_w = 1.0 + kappa_w * (1.0 - sqrt_Tr);
        double epsilon_w = epsilon_factor * alpha_w * alpha_w * inv_T;
	...
	// Derivatives of T_red; 
        // double dT_red_dT      = 1/Tcw;
	// double d2T_red_dT2    = 0;   

	//Derivatives of sqrt_Tr   
        double dsqrt_Tr_dT    =   0.5*(1.0/sqrt_Tr)*(1.0/Tcw) ;
        double d2sqrt_Tr_dT2  =   -0.25*( 1.0/(sqrt_Tr*T_red) )*(1.0/Tcw)*(1.0/Tcw);

        //Derivatives of kappa_w 
	//double kappa_w      =   kappa_0w + kappa_1w * (1.0 + sqrt_Tr) * (0.7 - T_red)   
	double dkappa_w_dT    =   kappa_1w * (dsqrt_Tr_dT) * (0.7 - T_red) + kappa_1w * (1.0 + sqrt_Tr) * (-1.0/Tcw) ;
	double d2kappa_w_dT2  =   kappa_1w * (d2sqrt_Tr_dT2) * (0.7 - T_red) + 2.0*kappa_1w * (dsqrt_Tr_dT) * (-1.0/Tcw) ;  			   	
        
	//Derivatives of alpha_w
	//double alpha_w      =   1.0 + kappa_w * (1.0 - sqrt_Tr);
	double dalpha_w_dT    =   dkappa_w_dT * (1.0 - sqrt_Tr) + kappa_w * (-dsqrt_Tr_dT);
	double d2alpha_w_dT2  =   d2kappa_w_dT2 * (1.0 - sqrt_Tr) + 2.0*dkappa_w_dT * (-dsqrt_Tr_dT) + kappa_w * (-d2sqrt_Tr_dT2);
			    

        if (degree >= 1){

            double depsilon_w_dT = epsilon_factor*( 2.0*alpha_w*dalpha_w_dT*inv_T - alpha_w*alpha_w*inv_T2 );
            ...
            if (degree == 2){
                ...
                double d2epsilon_w_dT2 = epsilon_factor*(   2.0*(dalpha_w_dT*dalpha_w_dT + alpha_w*d2alpha_w_dT2)*inv_T - 2.0*alpha_w*dalpha_w_dT*inv_T2 - ( 2.0*alpha_w*dalpha_w_dT*inv_T2 - 2.0*alpha_w*alpha_w*inv_T3 ) );
