...
//
void JetTester1D::populate_F(void *obj, int (*f)(void *obj, const RealVector &state, int degree, JetMatrix &jm),
    ...
    for (int i0 = 0; i0 < fine_subdivision[0]; i0++){
        ...
        (*f)(obj, point, 1, jm);
        fine_F(i0)  = jm.function();
        fine_JF(i0) = jm.Jacobian();
    }

    return;
}

//void JetTester1D::coarse_F(const MultiArray<RealVector> &fine_F, const MultiArray<DoubleMatrix> &fine_JF,
//                         int step,
...
void JetTester1D::populate_JF(void *obj, int (*f)(void *obj, const RealVector &state, int degree, JetMatrix &jm),
                            const RealVector &pmin, const RealVector &pmax,
...
//void JetTester1D::numerical_Jacobian(const MultiArray<RealVector>    &F, 
...
void JetTester1D::numerical_Hessian(const MultiArray<DoubleMatrix>               &JF, 
    ...
    for (int component = 0; component < rows; component++){
        for (int i0 = 1; i0 < range[0] - 1; i0++){ // The cycle step here and below should be powers of 2. The cycle's beginnig and end should take this into account.
            ...
            dev(0, 0) = std::abs((JF(i0 + 1)(component, 0) - JF(i0 - 1)(component, 0))*inv_2_delta[0] - HF(i0)[component](0, 0));
...
//void JetTester1D::test(void *obj, int (*f)(void *obj, const RealVector &state, int degree, JetMatrix &jm), 
